...
@app.post("/budgets")
def create_budget(budget: schemas.BudgetCreate, db: Session = Depends(get_db), user_id: int = Depends(get_current_user)):
...
def delete_receipt(receipt_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user)):
    r = db.query(models.Receipt).filter(models.Receipt.id == receipt_id, models.Receipt.user_id == user_id).first()
    if not r:
...
@app.post("/groups")
def create_group(group: schemas.GroupCreate, db: Session = Depends(get_db), user_id: int = Depends(get_current_user)):
    g = models.Group(name=group.name, created_by=user_id)
    db.add(g)
    db.commit()
    db.refresh(g)
    member = models.GroupMember(group_id=g.id, user_id=user_id)
    db.add(member)
    db.commit()
    return g
...
def get_groups(db: Session = Depends(get_db), user_id: int = Depends(get_current_user)):
    ...
    for g in groups:
        ...
        expenses = db.query(models.GroupExpense).filter(models.GroupExpense.group_id == g.id).all()
        total = sum(e.amount for e in expenses)
...
def add_group_expense(
    ...
    e = models.GroupExpense(group_id=group_id, paid_by=user_id, title=expense.title, amount=expense.amount)
    db.add(e)
    db.commit()
    db.refresh(e)
    return e
...
def get_settlements(group_id: int, db: Session = Depends(get_db), user_id: int = Depends(get_current_user)):
    ...
    for e in expenses:
        if e.paid_by != user_id:
            ...
            share = round(e.amount * (member_count - 1) / member_count, 2)
            settlements.append({
                "name": "Others",
                "owes": share,
                "direction": "owes-you",
